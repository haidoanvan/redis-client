...
import com.codergeezer.redisclient.logic.MyFlowLayout;
...
import com.codergeezer.redisclient.view.dialog.AddRowDialog;
...
public class ValueDisplayPanel extends JPanel {
  ...
  private final Map<Integer, String> pageIndexPointerMap;
  ...
  private KeyValueDisplayPanel parent;
                ...
                switch (type) {
                  ...
                  case "hash":
                    ...
                    ScanResult<Map.Entry<String, String>> hscanResult =
                        jedis.hscan(key, hashPointer, scanParams);
  ...
  private void initValuePreviewToolbarPanel() {
    ...
    keyTextField.addKeyListener(
        new KeyListener() {
          ...
          public void keyReleased(KeyEvent e) {
            keyTextField.setToolTipText(keyTextField.getText());
            renameButton.setEnabled(!keyTextField.getText().equals(key));
          }
        });
    ...
    ttlTextField.addKeyListener(
        new KeyListener() {
          ...
          public void keyReleased(KeyEvent event) {
            try {
              ttlButton.setEnabled(
                  Long.parseLong(ttlTextField.getText()) > 0
                      && !ttl.equals(Long.parseLong(ttlTextField.getText())));
            } catch (Exception e) {
              ttlButton.setEnabled(false);
            }
          }
    ...
    valuePreviewToolbarPanel.add(createReloadValueButton());
    valuePreviewToolbarPanel.add(createDeleteButton());
    ...
    if (node.isLeaf()) {
      ...
      if (userObject instanceof KeyInfo) {
        ...
        if (key.equals(nodeKey)) {
          ...
          redisManager.del(key, redisDatabase.getIndex());
          return node;
        }
      }
      return null;
  ...
  private void initValuePreviewPanel() {
    ...
    switch (typeEnum) {
      ...
      case List:
        ...
        tableModel =
            new DefaultTableModel(to2DimensionalArray(listValue), new String[] {"Row", "Value"});
        valueColumnIndex = 1;
        fieldOrScoreColumnIndex = 0;
        break;
      ...
      case Set:
        ...
        tableModel =
            new DefaultTableModel(to2DimensionalArray(setValue), new String[] {"Row", "Value"});
        valueColumnIndex = 1;
        fieldOrScoreColumnIndex = 0;
        break;
      ...
      case Zset:
        ...
        tableModel =
            new DefaultTableModel(
                zsetValueListTo2DimensionalArray(zsetValue),
                new String[] {"Row", "Score", "Value"});
        valueColumnIndex = 2;
      ...
      case Hash:
        ...
        tableModel =
            new DefaultTableModel(
                hashValueListTo2DimensionalArray(hashValue),
                new String[] {"Row", "Field", "Value"});
        valueColumnIndex = 2;
        fieldOrScoreColumnIndex = 1;
        break;
    ...
    ComboBox<ValueFormatEnum> valueFormatComboBox = new ComboBox<>(ValueFormatEnum.values());
    valueFormatComboBox.addItemListener(
        e -> {
          if (ItemEvent.SELECTED == e.getStateChange()) {
            ValueFormatEnum formatEnum = (ValueFormatEnum) e.getItem();
            valueTextArea =
                formatValue(project, valuePreviewAndFunctionPanel, formatEnum, valueTextArea);
          }
        });
    ...
    if (tableModel != null) {
      ...
      JBTable valueTable =
          createValueTable(
              tableModel, valueColumnIndex, fieldOrScoreColumnIndex, fieldTextArea, valueSizeLabel);
      ...
      valuePreviewSplitter.setDividerPositionStrategy(
      ...
      if (typeEnum == RedisValueTypeEnum.Hash) {
        ...
        keyValueSplitter.setDividerPositionStrategy(
            Splitter.DividerPositionStrategy.KEEP_FIRST_SIZE);
  ...
  private JPanel createPagePanel() {
    ...
    pagePanel.setLayout(new MyFlowLayout());
  ...
  @NotNull
  private JBTable createValueTable(
      DefaultTableModel tableModel,
      int valueColumnIndex,
      int fieldOrScoreColumnIndex,
      JBTextArea fieldTextArea,
      JBLabel valueSizeLabel) {
    JBTable valueTable =
        new JBTable(tableModel) {
          @Override
  ...
  private JButton createAddRowButton() {
    ...
    addRowButton.addActionListener(
        e -> {
          ...
          addRowDialog.setCustomOkAction(
              actionEvent -> {
                try (Jedis jedis = redisManager.getJedis(redisDatabase.getIndex())) {
                  if (!jedis.exists(key)) {
                    ErrorDialog.show(String.format("No such key: %s", key));
                    ...
                    switch (typeEnum) {
                      ...
                      case Zset:
                        jedis.zadd(
                            key,
                            Double.parseDouble(addRowDialog.getScoreOrField()),
                            addRowDialog.getValue());
                        break;
  ...
  private JButton createSaveValueButton(JBTextArea fieldTextArea) {
    ...
    saveValueButton.addActionListener(
        e -> {
          ...
          new ConfirmDialog(
                  ...
                  actionEvent -> saveNewValue(newField, newValue))
              .show();
        });
                    ...
                    try (Jedis jedis = redisManager.getJedis(redisDatabase.getIndex())) {
                      ...
                      if (renamenx == 0) {
                        ...
                        keyTreeDisplayPanel.renderKeyTree(
                            parent.getKeyFilter(), parent.getGroupSymbol());
                      }
                    } catch (Exception exception) {
  ...
  private JButton createReloadValueButton() {
    ...
    reloadButton.addActionListener(e -> doReload());
    return reloadButton;
  }

  private void doReload() {
    init(project, parent, keyTreeDisplayPanel, key, redisManager, redisDatabase, loadingDecorator);
    updateSelected(-1, "", "");
    this.updateUI();
  }

  private void updateValueSize(JLabel valueSizeLabel) {
    valueSizeLabel.setText(
        "Value size: " + getSelectedValue().getBytes(StandardCharsets.UTF_8).length + "bytes");
  }
